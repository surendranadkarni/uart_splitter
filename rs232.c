...
/*
char comports[30][16]={"/dev/ttyUSB0",
...
#define IS_HW_PORT(dev) (strncmp(dev, "/dev/pts", 8))

int RS232_OpenComport(char *file, int baudrate)
{
  int baudr, status;
  ...
  comport_number = open(file, O_RDWR | O_NOCTTY | O_NDELAY);
  if(comport_number ==-1)
  {
...
#if 0
    TODO: restore old port
  error = tcgetattr(comport_number, old_port_settings + comport_number);
  if(error==-1)
  {
  ...
  error = tcsetattr(comport_number, TCSANOW, &new_port_settings);
  if(IS_HW_PORT(file))
  {
    if(error==-1)
    {
    ...
    if(ioctl(comport_number, TIOCMGET, &status) == -1)
    {
    ...
    if(ioctl(comport_number, TIOCMSET, &status) == -1)
    {
...
static int wait_on_read(int fd)
{
    fd_set rfds;
    struct timeval tv;
    int retval;
...
{
  int n = 0;
  if(wait_on_read(comport_number) != -1)
  {
